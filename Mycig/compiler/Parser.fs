...
type private Assoc = Associativity
...
type Parser() =
    ...
    let zblock p =
        between
            (spaces .>> pchar '{' .>> spaces)
            (spaces .>> pchar '}' .>> spaces)
            (pstring "" |>> (fun _ -> ss.createScope ()) >>. p
             |>> (fun x ->
                 ss.deleteScope ()
                 x))
    ...
    let funcBlock p =
        between (spaces .>> pchar '{' .>> spaces) (spaces .>> pchar '}' .>> spaces) (many p)

    let funcBlock1 p =
        between (spaces .>> pchar '{' .>> spaces) (spaces .>> pchar '}' .>> spaces) (many1 p)

    let blockOrExp =
        choice [ block1 funcTerm
    ...
    let let_ =
        pipe2
            getPosition
            (pstring "let" .>> spaces1
             >>. choice [ stringReturn "mut" true
                          pstring "" >>% false ]
             .>> spaces
             .>>. choice [ stringReturn "*" true .>> spaces
                           pstring "" >>% false ]
             .>>. ident
    ...
    let func_ =
        pipe2
            getPosition
            (opt (stringReturn "pub" true .>> spaces1)
             .>> pstring "func"
             .>> spaces1
             .>>. ident
             .>>. between
                      ...
                      (spaces .>> pchar ')')
                      (sepBy
                          (spaces >>. opt (attempt (stringReturn "*" 0uy))
                           .>>. ident
                           .>>. opt (attempt (spaces .>> pchar ':' .>> spaces >>. typp)))
                          (spaces .>> pchar ',')
                       |>> (fun lst ->
                           ...
                           lst
                           |> List.iter (fun ((_, name), _) -> ss.add name)

                           lst))
             .>>. opt (
                 attempt (
                     spaces .>> pstring "->" .>> spaces >>. typp
                     .>> spaces
                 )
             )
             .>>. funcBlock1 funcTerm
            ...
            (pstring "abs"
             ...
             .>>. between
                      ...
                      (sepBy
                       ...
                       |>> (fun lst ->
                           ss.createScope ()
                           lst |> List.iter (fun (name, _) -> ss.add name)
                           lst))
             .>>. (spaces .>> pstring "->" .>> spaces >>. typp
                   .>> spaces)
             .>> funcEndLines)
                    ...
                    { Type = "func_impl_abs"
                      ...
                      Data =
                        sprintf
                            ...
                            rettyp
                            (args
                             |> List.map (fun (f, s) -> sprintf "arr: [str: \"%s\", ref: %i]" f s)
                             |> String.concat ", ") })
    ...
    let impl_ =
        pipe2
            getPosition
            (pstring "impl" .>> spaces1 >>. ident
             .>>. block (choice [ attempt initf; func_impl ]))
                      ...
                      Data =
                        sprintf
                            ...
                            ((match frames with
                              | Some lst -> lst
                              | None -> [])
                             |> List.map (fun (idt, name) -> sprintf "arr: [bool: %b, str: \"%s\"]" idt.IsSome name)
                             |> String.concat ", ")
                            (content
                             |> List.map (sprintf "ref: %i")
                             |> String.concat ", ") })
    ...
    do
        ...
        let adjustPosition offset (pos: Position) =
            Position(pos.StreamName, pos.Index + int64 offset, pos.Line, pos.Column + int64 offset)
        ...
        let addOpr name prece assoc mapping =
            let op =
                InfixOperator(
                    ...
                    assoc,
                    (),
                    fun opPos lhs rhs -> mapping (adjustPosition -name.Length opPos) lhs rhs
                )
                    ...
                    (fun pos lhs rhs ->
                        fast.add
                            { Type = "operator_" + name
                              Line = pos.Line
                              Column = pos.Column
                              Data = sprintf "[ref: %i, ref: %i]" lhs rhs })))

        funcTermRef.Value <-
            choice [ func_ .>> funcEndLines
                     let_ .>> endLines
                     if_ .>> endLines
                     expr .>> endLines ]
            .>> endLines
            ...
            choice [ attempt (
                         pipe2
                             ...
                             ((choice [ attempt (pstring "&" .>> spaces .>>. pstring "mut" .>> spaces1 >>% "&mut")
                              ...
                              .>>. typ)
                             (fun pos (prefix, inner) ->
                                 fast.add
                                     { Type = "type_bw_" + prefix
                                       Line = pos.Line
                                       Column = pos.Column
                                       Data = sprintf "[ref: %i]" inner })
                     )
                     attempt (
                         pipe3
                             getPosition
                                   ...
                                   Data =
                                     sprintf
                                         ...
                                         (match rettyp with
                                          | Some t -> t
                                          | None -> -1) })
                     pipe2 getPosition (pstring "abs" .>> spaces1 >>. typ) (fun pos t ->
                         fast.add
                             { Type = "type_abs"
                               Line = pos.Line
                               Column = pos.Column
                               Data = sprintf "[ref: %i]" t })
                     pipe2 getPosition ident (fun pos s ->
                         if s = "_" then
                             -1
                         else
                             fast.add
        ...
        exprTermref.Value <-
            choice [ attempt (
                         pipe2 getPosition (pfloat .>> pstring "f64") (fun pos value ->
                             fast.add
                                 { Type = "operand_f64"
                                   Line = pos.Line
                                   Column = pos.Column
                     ...
                     attempt (
                         pipe2 getPosition (pint8 .>> pstring "i8") (fun pos value ->
                             fast.add
                                 { Type = "operand_i8"
                                   Line = pos.Line
                                   Column = pos.Column
                     ...
                     attempt (
                         pipe2 getPosition (pint16 .>> pstring "i16") (fun pos value ->
                             fast.add
                                 { Type = "operand_i16"
                                   Line = pos.Line
                                   Column = pos.Column
                     ...
                     attempt (
                         pipe2 getPosition (pint64 .>> pstring "i64") (fun pos value ->
                             fast.add
                                 { Type = "operand_i64"
                                   Line = pos.Line
                                   Column = pos.Column
                     ...
                     attempt (
                         pipe2 getPosition (puint8 .>> pstring "u8") (fun pos value ->
                             fast.add
                                 { Type = "operand_u8"
                                   Line = pos.Line
                                   Column = pos.Column
                     ...
                     attempt (
                         pipe2 getPosition (puint16 .>> pstring "u16") (fun pos value ->
                             fast.add
                                 { Type = "operand_u16"
                                   Line = pos.Line
                                   Column = pos.Column
                     ...
                     attempt (
                         pipe2 getPosition (puint32 .>> pstring "u32") (fun pos value ->
                             fast.add
                                 { Type = "operand_u32"
                                   Line = pos.Line
                                   Column = pos.Column
                     ...
                     attempt (
                         pipe2 getPosition (puint64 .>> pstring "u64") (fun pos value ->
                             fast.add
                                 { Type = "operand_u64"
                                   Line = pos.Line
                                   Column = pos.Column
                     ...
                     pipe2 getPosition (between (pchar '\'') (pchar '\'') (regex @"\\?.")) (fun pos value ->
                         fast.add
                             { Type = "operand_char"
                               Line = pos.Line
                               Column = pos.Column
                     ...
                     attempt (
                         ...
                         >>= (fun (pos, vName) ->
                             if ss.content vName then
                                 ...
                                 fail
                                 <| sprintf "variable '%s' does not exist." vName))
                     pipe2
                         getPosition
                         ((sepBy1 ident (attempt (spaces .>> pstring "::" .>> spaces)))
                          .>>. between
                                   ...
                                   Data =
                                     sprintf
                                         ...
                                         (args
                                          |> List.map (sprintf "ref: %i")
                                          |> String.concat ", ") })
                     pipe2 getPosition ident (fun pos name ->
                         fast.add
                             { Type = "ident"
                               Line = pos.Line
                               Column = pos.Column
                               Data = sprintf "[str: \"%s\"]" name })
                     pipe2 getPosition (between (pchar '(' .>> spaces) (spaces .>> pchar ')') expr) (fun pos expr ->
                         fast.add
                             { Type = "paren"
                               Line = pos.Line
                               Column = pos.Column
                               Data = sprintf "[ref: %i]" expr })
                     pipe2 getPosition (block1 funcTerm) (fun pos content ->
                         fast.add
                             { Type = "block"
                               Line = pos.Line
                               Column = pos.Column
